...
public class SubprojectDao {
	...
	public int subprojectAdd(Connection con, Subproject subproject)throws Exception {
		...
		pstmt.setInt(3, subproject.getItemTypeId());
	...
	public int subprojectUpdate(Connection con, Subproject subproject)throws Exception {
		String sql = "update t_subproject set subNum=?,name=?,itemTypeId=?,developerName=?,state=?,tel=? where subprojectId=?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setString(1, subproject.getSubNumber());
		pstmt.setString(2, subproject.getName());
		pstmt.setInt(3, subproject.getItemTypeId());
		...
		pstmt.setString(5, subproject.getState());
		pstmt.setString(6, subproject.getTel());
		pstmt.setInt(7, subproject.getSubprojectId());
		return pstmt.executeUpdate();
	}
}
